...
from common import extract_dangerous_function_info


def enrich_findings(findings):
    for f in findings:
        sink = f.get("sink") or {}
        code = sink.get("code") or ""
        info = extract_dangerous_function_info(code)
        if not info:
            continue
        f["dangerous_function"] = info.get("name")
        f["recommendation"] = info.get("alternatives", [])
        note = info.get("note")
        if note:
            existing = f.get("notes") or ""
            f["notes"] = (existing + " " + note).strip()
    return findings
...
def main() -> None:
    ...
    new_findings = extract_findings_from_traces(traces, ["rce"], "Possible RCE", "RCE")
    new_findings = enrich_findings(new_findings)
    new_findings = apply_rule_audit_quick_filter(new_findings, "rce_audit")

    out_dir = os.path.join(out_root, "rce_audit")
