...
在完成框架指纹识别（模块 2）和 PHP 版本推断（模块 3）后，将检测到的版本信息与 `shared/known_cves.md` 交叉比对，生成版本安全预判结果。

### 执行步骤

1. **提取版本三元组**: `(framework, framework_version, php_version)`
2. **交叉比对 known_cves.md**:
   - 按框架类型匹配章节（Laravel / ThinkPHP / WordPress / PHP Core 等）
   - 按版本号范围匹配已知 CVE（精确到小版本号段）
   - 记录每个匹配的 CVE 的严重等级和受影响的审计方向
3. **生成 version_alerts 数组**:
   - 每个匹配项包含: `component`, `detected_version`, `cve_id`, `severity`, `description`, `affected_auditors`
   - severity 按 CVSS 分级: critical (≥9.0), high (7.0-8.9), medium (4.0-6.9), low (<4.0)
4. **优先级标记**:
   - critical/high CVE → 在输出中标注 `⚠️ 版本预判: 存在已知高危漏洞`
   - 映射 affected_auditors（如 ThinkPHP 5.x RCE → `["rce_auditor"]`）

### 版本-CVE 映射规则

| 组件 | 版本范围 | 典型 CVE | 受影响 Auditor |
|------|----------|----------|----------------|
...
| WordPress | < 6.0 | 多个 Core CVE | wordpress_auditor |
| PHP | < 5.3.4 | Null Byte LFI | lfi_auditor |
| PHP | < 8.0 | Type Juggling | authz_auditor, crypto_auditor |

> 完整映射以 `shared/known_cves.md` 为准，此表仅为示例。

### 输出格式

`version_alerts` 数组写入 `environment_status.json`，遵循 `schemas/environment_status.schema.json` 定义。
