...
对所有传入目标函数的参数进行注入。测试 GET 和 POST 参数。扫描常见内部端口：80, 443, 8080, 8443, 3306, 6379, 5432, 11211, 27017。
...
对 ssrf-target 解析其 IP 并应用相同编码变体。对 URL 验证过滤器测试每种形式。
...
直接访问被阻止时对 169.254.169.254 应用 R2 的 IP 编码。检查 IMDSv2（AWS）是否需要先 PUT 获取 Token。
...
利用 `parse_url()`、`filter_var()` 和 cURL 实际 URL 处理之间的差异。测试每个变体，比较验证器看到的内容与 HTTP 客户端实际请求的内容。
...
3. 每轮测试所有已识别的注入点和参数向量
4. 疑似 Blind SSRF 时使用带外检测（DNS 回调、HTTP 回调）
5. 记录每个请求、响应和时序观察
6. 确认后记录完整 Payload 和响应摘录
...
```
[已确认] SSRF - 第 X 轮
端点: GET /fetch.php?url=PAYLOAD
参数: url
Payload: http://ssrf-target:80/
物证: 响应包含 ssrf-target 内部页面内容
严重程度: 严重
修复方案: 白名单限制允许的 URL/域名。禁用非必要协议。验证解析后的 IP 是否属于内部范围。
```
...
### 共享写入
发现内网服务/端点时**必须**写入 `$WORK_DIR/shared_findings.jsonl`:
- 可达的内部服务 → `finding_type: internal_url`
- 云元数据获取的凭证 → `finding_type: credential`

### 共享读取
攻击阶段开始前读取 `shared_findings.jsonl`，利用信息泄露发现的内网 IP。
...
当 SSRF 漏洞确认后，下一步是识别并利用内部网络中运行的未授权服务。以下是最常见的高价值目标及其完整攻击链。
