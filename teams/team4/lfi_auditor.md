...
对所有传入目标函数的参数进行注入。测试 GET、POST 和 Cookie 向量。`../` 深度从 3 到 10 层递增。响应包含 `root:x:0:0` 即确认。
...
将各编码变体应用到 R1 的遍历路径上。测试完整编码和部分编码。
...
解码 Base64 响应验证是否包含 PHP 源码。枚举常见文件名：index.php、config.php、db.php、.env、wp-config.php。
...
利用应用追加 `.php` 或其他扩展名的场景。空字节在 OS 层截断字符串。仅在 PHP 版本 < 5.3.4 或版本未知时测试。
...
响应中出现 `uid=` 即确认代码执行。可结合 R2 编码绕过直接路径过滤。
...
同时测试 `/proc/self/fd/0` 到 `/proc/self/fd/10` 的文件描述符包含。
...
```
[已确认] LFI - 第 X 轮
端点: GET /page.php?file=PAYLOAD
参数: file
Payload: ../../../etc/passwd
物证: root:x:0:0:root:/root:/bin/bash
严重程度: 严重
修复方案: 使用文件名白名单。禁止将用户输入直接传递给 include/require。
```
...
### 共享读取
攻击阶段开始前读取 `shared_findings.jsonl`，利用其他审计员发现的文件路径和绕过方法。
