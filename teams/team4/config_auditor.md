...
`OPTIONS /.env`, `TRACE /.env`, `HEAD /admin`, `PROPFIND /`（WebDAV 列举）, `MOVE`/`COPY` 操作。检查 TRACE 是否反射头部（XST）。
...
检查 `Access-Control-Allow-Origin` 是否反射攻击者 Origin 且 `Access-Control-Allow-Credentials: true`。
...
- 检查 `NXDOMAIN` 或特定错误页面标识
- 检查 `A` 记录指向的 IP 是否仍归属目标

### R12 - PHP 运行时配置审计

检查 `php.ini` / `phpinfo()` 中的危险配置:
...
```json
{
  ...
  "sub_type": "sensitive_file|debug_leak|missing_header|cors|default_creds|cookie_security",
  "round": 1,
  "endpoint": "GET /.env",
  "evidence": "响应包含 APP_KEY=base64:xYz..., DB_PASSWORD=production_pass",
  "confidence": "confirmed|highly_suspected|potential_risk",
  "impact": "凭证泄露|完全数据库访问|会话劫持",
  "remediation": "在服务器配置中阻止敏感路径，禁用调试，设置安全头，修改默认凭证"
}
```
...
### 共享写入
发现以下信息时**必须**追加写入 `$WORK_DIR/shared_findings.jsonl`:
...
### 共享读取
攻击阶段开始前读取 `shared_findings.jsonl`，利用信息泄露审计员发现的内部路径。
...
- 标记为 `potential_risk`，继续检查是否存在动态 Origin 反射
...
**Detection Rule:**
1. 发送 `Origin: https://evil-attacker.com`，检查响应是否反射该 Origin
...
**Detection Rule:**
- 发送 `Origin: null`，检查是否返回 `Access-Control-Allow-Origin: null`
- 发送 `Origin: https://target.com.evil.com`，检查 Origin 是否被接受
- 发送 `Origin: https://evil-target.com`，检查前缀/后缀匹配绕过
- 发送 `Origin: https://sub.target.com`（不存在的子域名），检查是否被信任
