...
- `$WORK_DIR/environment_status.json`（确认 framework=WordPress）
...
5. 检查 `wp-config.php` 安全常量:
   - `DISALLOW_FILE_EDIT` — 禁用后台文件编辑
   - `DISALLOW_FILE_MODS` — 禁用插件/主题安装
   - `FORCE_SSL_ADMIN` — 强制后台 HTTPS
   - `WP_DEBUG` — 调试模式
6. 检查用户角色: Administrator, Editor, Author, Contributor, Subscriber
